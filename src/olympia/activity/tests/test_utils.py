...
sample_email = os.path.join(TESTS_DIR, 'emails', 'email.txt')
multi_email = os.path.join(TESTS_DIR, 'emails', 'email_multipart.txt')
quoted_email = os.path.join(TESTS_DIR, 'emails', 'email_quotedprintable.txt')
attach_email = os.path.join(TESTS_DIR, 'emails', 'email_attachment.txt')
...
class TestEmailParser(TestCase):
    ...
    def test_multipart(self):
        ...
        parser = ActivityEmailParser(payload)
        assert parser.get_body() == 'this is the body text\n'
        assert parser.get_uuid() == 'abc123'

    def test_quoted_printable(self):
        email = open(quoted_email).read()
        payload = base64.standard_b64encode(email)
        parser = ActivityEmailParser(payload)

        body = parser.get_body()
        assert ('Thanks for comments. Here is our answers:\n\n'
                '1) We removed analytics.' in body)
        assert '=20' not in body
        assert 'amo-editors@mozilla.org' not in body
    ...
    def test_with_attachments(self):
        ...
        body = parser.get_body()
        assert 'a reply with an attachment' in body
        assert parser.get_uuid() == 'abc123'
...
class TestAddEmailToActivityLog(TestCase):
    ...
    def test_developer_comment(self):
        ...
        note = add_email_to_activity_log(self.email_base64)
    ...
    def test_with_max_count_token(self):
        ...
        assert not add_email_to_activity_log(self.email_base64)
        self.token.refresh_from_db()
...
class TestLogAndNotify(TestCase):
    ...
    def _recipients(self, email_mock):
        ...
        for call in email_mock.call_args_list:
            ...
            assert reply_to.endswith(settings.POSTFIX_DOMAIN)
        return recipients
...
def test_send_activity_mail():
    ...
    reply_email = 'reviewreply+%s@%s' % (uuid, settings.POSTFIX_DOMAIN)
