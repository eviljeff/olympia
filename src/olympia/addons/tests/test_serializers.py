...
class AddonSerializerOutputTestMixin(object):
    ...
    def test_basic(self):
        ...
        assert result['homepage'] == {
            'en-US': text_type(self.addon.homepage),
        }
        ...
        assert result['support_url'] == {
            'en-US': text_type(self.addon.support_url),
        }
    ...
    def test_wrap_outgoing_links(self):
        ...
        assert result['contributions_url'] == (
            get_outgoing_url(text_type(self.addon.contributions)))
        assert result['homepage'] == {
            'en-US': get_outgoing_url(text_type(self.addon.homepage)),
        }
        assert result['support_url'] == {
            'en-US': get_outgoing_url(text_type(self.addon.support_url)),
        }
        ...
        assert result['contributions_url'] == (
            get_outgoing_url(text_type(self.addon.contributions)))
        assert result['homepage'] == (
            get_outgoing_url(text_type(self.addon.homepage))
        )
        assert result['support_url'] == (
            get_outgoing_url(text_type(self.addon.support_url))
        )
...
class TestVersionSerializerOutput(TestCase):
    ...
    def test_builtin_license(self):
        ...
        assert result['name'] == {
            'en-US': text_type(LICENSES_BY_BUILTIN[18].name)}
        ...
        assert result['name'] == {
            'de': text_type(LICENSES_BY_BUILTIN[18].name)}
        ...
        assert result['name'] == text_type(LICENSES_BY_BUILTIN[18].name)
...
class TestESAddonAutoCompleteSerializer(ESTestCase):
    ...
    def test_basic(self):
        ...
        assert result['id'] == self.addon.pk
        assert result['name'] == {'en-US': text_type(self.addon.name)}
        assert result['icon_url'] == absolutify(self.addon.get_icon_url(64))
        assert result['type'] == 'extension'
        assert result['url'] == absolutify(self.addon.get_url_path())
