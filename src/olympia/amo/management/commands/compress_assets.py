...
def path(*args):
    return os.path.join(settings.JINGO_MINIFY_ROOT, *args)
...
class Command(BaseCommand):  # pragma: no cover
    ...
    minify_skipped = 0
    cmd_errors = False
    ext_media_path = os.path.join(settings.JINGO_MINIFY_ROOT, 'external')
    ...
    def _preprocess_file(self, filename):
        ...
        found_path = static_finder(filename)
        # If the path is not found by Django's static finder (like we are
        # trying to get an output path), it returns None, so fall back.
        full_path = found_path or os.path.join(
            settings.JINGO_MINIFY_ROOT, filename)

        if css_bin:
            self._call('%s %s %s.css' % (css_bin, full_path, full_path),
                       shell=True, stdout=PIPE)
            full_path = '%s.css' % full_path
        elif filename.endswith('.styl'):
            self._call('%s --include-css --include %s < %s > %s.css' %
                       (settings.STYLUS_BIN, os.path.dirname(full_path),
                        full_path, full_path),
                       shell=True, stdout=PIPE)
            full_path = '%s.css' % full_path
        return full_path
    ...
    def _is_changed(self, concatted_file):
        ...
        if ((os.path.exists(concatted_file) and
             os.path.getsize(concatted_file) ==
    ...
    def _cachebust(self, css_file, bundle_name):
        ...
        css_parsed = re.sub('url\(([^)]*?)\)',
                            lambda url: self._cachebust_regex(url, css_file),
                            css_content)
    ...
    def _minify(self, ftype, file_in, file_out):
        ...
        if ftype == 'js' and hasattr(settings, 'UGLIFY_BIN'):
            ...
            minify_str = '%s %s -o %s %s -m'
            self._call(minify_str % (o['bin'], self.v, file_out, file_in),
                       shell=True, stdout=PIPE)
