...
class TestCompressAssetsCommand(TestCase):
    ...
    def test_js_helper(self, getmtime, time):
        ...
        assert s == (
            '<script src="js/test.js?build=1"></script>\n'
    ...
    @mock.patch('jingo_minify.helpers.time.time')
    @mock.patch('jingo_minify.helpers.os.path.getmtime')
    def test_css_helper(self, getmtime, time):
        """
        ...
        t = env.from_string(
            "{{ css('common_protocol_less_url', debug=True) }}")
        s = t.render()

        assert s == ('<link rel="stylesheet" media="screen,projection,tv" '
    ...
    def test_inline_css_helper(self):
        env = jingo.get_env()
        t = env.from_string("{{ inline_css('common', debug=True) }}")
        s = t.render()

        assert s == ('<style type="text/css" media="screen,projection,tv">'
                     'body {\n    color: #999;\n}\n</style>')

        t = env.from_string("{{ inline_css('common', debug=False) }}")
        s = t.render()

        assert s == ('<style type="text/css" media="screen,projection,tv">body'
                     '{color:#999}</style>')

    def test_inline_css_helper_multiple_files(self):
        env = jingo.get_env()
        t = env.from_string("{{ inline_css('common_multi', debug=True) }}")
        s = t.render()

        assert s == (
            '<style type="text/css" media="screen,projection,tv">body {\n    '
    ...
    def test_inline_css_helper_external_url(self):
        env = jingo.get_env()
    ...
    @mock.patch('jingo_minify.helpers.time.time')
    @mock.patch('jingo_minify.helpers.os.path.getmtime')
    def test_css(self, getmtime, time):
        getmtime.return_value = 1
        time.return_value = 1
        env = jingo.get_env()
    ...
    def test_compiled_css(self, open_mock, subprocess_mock, getmtime_mock,
                          time_mock):
        jingo.get_env().from_string(
            "{{ css('compiled', debug=True) }}").render()

        assert subprocess_mock.Popen.mock_calls == [
            mock.call(['lessc-bin', 'static/css/less.less'], stdout=mock.ANY),
            mock.call(['sass-bin', 'static/css/sass.sass'], stdout=mock.ANY),
            mock.call(['sass-bin', 'static/css/scss.scss'], stdout=mock.ANY)]
    ...
    @mock.patch('jingo_minify.helpers.time.time')
    @mock.patch('jingo_minify.helpers.os.path.getmtime')
    def test_js(self, getmtime, time):
        getmtime.return_value = 1
        time.return_value = 1
        env = jingo.get_env()
    ...
    @override_settings(
        MINIFY_BUNDLES={'css': {'common_multi': ['css/test.css',
                                                 'css/test2.css']}}
    )
    @mock.patch('jingo_minify.helpers.subprocess')
    def test_compress_assets_command_with_git(self, subprocess_mock):
    ...
    @mock.patch('jingo_minify.helpers.subprocess')
    def test_compress_assets_command_without_git(self, subprocess_mock):
        build_id_file = os.path.realpath(
            os.path.join(settings.ROOT, 'build.py'))
        try:
            os.remove(build_id_file)
        ...
        # Call command a second time. We should get a different build id, since
        # it depends on a uuid.
        call_command('compress_assets', use_uuid=True)
        with open(build_id_file) as f:
            contents_after = f.read()

        assert not contents_before == contents_after
