...
env = load_env(ENVIRON_SETTINGS_FILE_PATH)
...
class BaseSetting:
    ...
    def apply(cls, to):
        """
        Run as `SettingsSubClass.apply(globals())`
        where SettingsSubClass is the subclass of Setting/BaseSetting.
        """
        instance = cls()
        to.update({
            member: getattr(instance, member)
            for member in dir(instance) if member.isupper()
        })

    def _build_id(self):
        try:
            # If we have build ids available, we'll grab them here and add them
            # to our CACHE_KEY_PREFIX. This will let us not have to flush
            # memcache during updates and it will let us preload data into it
...
class Settings(BaseSetting):
    ...
    def _path(self, *folders):
        return os.path.join(self.ROOT, *folders)
    ...
    SILENCED_SYSTEM_CHECKS = (
        # Recommendation to use OneToOneField instead of
        # ForeignKey(unique=True) but our translations are the way they are...
        'fields.W342',
    )

    # LESS CSS OPTIONS (Debug only).
    LESS_PREPROCESS = True  # Compile LESS with Node instead of client-side JS?
    ...
    def CORS_URLS_REGEX(self):
        return r'{}(?!accounts/session/)'.format(self.DRF_API_REGEX)

    def get_db_config(self, environ_var, atomic_requests=True):
        values = env.db(
            var=environ_var,
        ...
        values.update({
            ...
            # `atomic_requests` should be `False` for database replicas where
            # no write operations will ever happen.
    ...
    AMO_LANGUAGES = olympia.core.languages.LANGUAGE_MAPPING.keys()
    ...
    LANGUAGES = {
        ...
        for locale, value in olympia.core.languages.LANGUAGE_MAPPING.items()}

    @property
    def LANGUAGE_URL_MAP(self):
        return {
            locale.lower(): locale for locale in self.AMO_LANGUAGES}

    @property
    def LOCALE_PATHS(self):
        return (self._path('locale'),)

    # If you set this to False, Django will make some optimizations so as not
    # to load the internationalization machinery.
    USE_I18N = True

    # The host currently running the site.  Only use this in code for good
    # reason; the site is designed to run on a cluster and should continue to
    # support that
    HOSTNAME = socket.gethostname()

    # The front end domain of the site. If you're not running on a cluster this
    # might be the same as HOSTNAME but don't depend on that.  Use this when
    # you need the real domain.
    @property
    def DOMAIN(self):
    ...
    # The base URL for the external user-facing frontend.  Only really useful
    # for the internal admin instances of addons-server that don't run
    # addons-frontend.
    @property
    def EXTERNAL_SITE_URL(self):
        return env('EXTERNAL_SITE_URL', default=self.SITE_URL)

    # Domain of the services site.  This is where your API, and in-product
    # pages live.
    @property
    def SERVICES_DOMAIN(self):
        return f'services.{self.DOMAIN}'

    # Full URL to your API service. No trailing slash.
    #   Example: https://services.addons.mozilla.org
    @property
    def SERVICES_URL(self):
        return f'http://{self.SERVICES_DOMAIN}'

    # URL of the code-manager site, see:
    # https://github.com/mozilla/addons-code-manager
    @property
    def CODE_MANAGER_URL(self):
        return f'https://code.{self.DOMAIN}'

    # Filter IP addresses of allowed clients that can post email through the
    # API.
    ALLOWED_CLIENTS_EMAIL_API = env.list(
        'ALLOWED_CLIENTS_EMAIL_API', default=[])
    ...
    INBOUND_EMAIL_VALIDATION_KEY = env(
        'INBOUND_EMAIL_VALIDATION_KEY', default='')
    # Domain emails should be sent to.
    @property
    def INBOUND_EMAIL_DOMAIN(self):
        return env('INBOUND_EMAIL_DOMAIN', default=self.DOMAIN)
    ...
    @property
    def SUPPORTED_NONAPPS_NONLOCALES_REGEX(self):
        return self.DRF_API_REGEX

    # Paths that don't require an app prefix
    # This needs to be kept in sync with addons-frontend's
    # validClientAppUrlExceptions
    # github.com/mozilla/addons-frontend/blob/master/config/default-amo.js
    SUPPORTED_NONAPPS = (
        ...
        return [
            {
                ...
                'APP_DIRS': True,
                'DIRS': (
                    self._path('media', 'docs'),
                    self._path('src/olympia/templates'),
                ),
                'OPTIONS': {
                    # jinja.pocoo.org/docs/dev/extensions/#newstyle-gettext
                    ...
                    'match_regex': r'^(?!({paths})).*'.format(
                        paths='|'.join(self.JINJA_EXCLUDE_TEMPLATE_PATHS)),
                    'context_processors': (
    ...
    # we need to know that we're running on either port 80 or 443, or do
    # something with SECURE_PROXY_SSL_HEADER but we shouldn't if we can avoid
    # that. So, let's simply grab the properly set `X-Forwarded-Port` header.
    # github.com/mozilla/addons-server/issues/8835#issuecomment-405340432
    #
    # This is also backwards compatible for our local setup since Django falls
    # back to using `Port` if `X-Forwarded-Port` isn't set.
    USE_X_FORWARDED_PORT = True

    MIDDLEWARE = (
        # Our middleware to make safe requests non-atomic needs to be at the
        # top.
        ...
        # AMO URL middleware as high as possible to get locale/app aware URLs.
        ...
        # current user) and after SetRemoteAddrFromForwardedFor (to get the
        # correct IP).
    ...
    # This needs to point to prod, because that's where the database lives. You
    # can change it locally to test the extraction process, but be careful not
    # to accidentally nuke translations when doing that!
    ...
    DISCOVERY_EDITORIAL_CONTENT_FILENAME = (
        ...
        return {
            ...
            'DOMAIN_METHODS': {
                'django': [
                    ...
                    # Extract the generated file containing editorial content
                    # for all disco pane recommendations using jinja2 parser.
                    # It's not a real template, but it uses jinja2 syntax for
                    # convenience, hence why it's not in templates/ with a
                    # .html extension.
                    (self.DISCOVERY_EDITORIAL_CONTENT_FILENAME, 'jinja2'),

                    # Make sure we're parsing django-admin templates with the
                    # django template extractor
                    (
    ...
    # Bundles is a dictionary of two dictionaries, css and js, which list css
    # files and js files that can be bundled together by the minify app.
    MINIFY_BUNDLES = {
        'css': {
            'restyle/css': (
    ...
    # Prefix for cache keys (will prevent collisions when running parallel
    # copies). This value is being used by `conf/settings/{dev,stage,prod}.py
    def CACHE_KEY_PREFIX(self):
        return f'amo:{self.build_id()}:'

    @property
    def CACHE_MIDDLEWARE_KEY_PREFIX(self):
        return self.CACHE_KEY_PREFIX
    FETCH_BY_ID = True

    # Number of seconds a count() query should be cached.  Keep it short
    # because it's not possible to invalidate these queries.
    ...
    # File paths
    @property
    def ADDON_ICONS_DEFAULT_PATH(self):
        return os.path.join(self.ROOT, 'static', 'img', 'addon-icons')
    ...
    # This value must be kept in sync with authTokenValidFor from
    # addons-frontend:
    # https://github.com/mozilla/addons-frontend/blob/2f480b474fe13a676237fe76a1b2a057e4a2aac7/config/default-amo.js#L111  # NOQA
    ...
    def DEFAULT_FROM_EMAIL(self):
        return self.ADDONS_EMAIL

    # Email goes to console by default.  s/console/smtp/ for regular delivery
    ...
    EMAIL_DENY_LIST = env.list(
        'EMAIL_DENY_LIST', default=('nobody@mozilla.org',))
    ...
    # We have separate celeryds for processing devhub & images as fast as
    # possible. Some notes:
    ...
    CELERY_TASK_ROUTES = {
        # Priority.
        # If your tasks need to be run as soon as possible, add them here so
        # they are routed to the priority queue.
        ...
        'olympia.devhub.tasks.handle_upload_validation_result': {
            'queue': 'devhub'},
        'olympia.devhub.tasks.revoke_and_regenerate_api_key': {
            'queue': 'devhub'},
        ...
        # This puts the chord_unlock task on the devhub queue. Which means
        # anything that uses chord() or group() must also be running in this
        # queue or must be on a worker that listens to the same queue.
        ...
        # Crons
        'olympia.addons.tasks.update_addon_average_daily_users': {
            'queue': 'cron'},
    ...
    # The task can catch that and recover but should exit ASAP. Note that there
    # is a separate, shorter timeout for validation tasks.
    ...
    # at the VERY BOTTOM: Settings.read_only_mode(globals())
    @staticmethod
    ...
    @property
    def MAX_PHOTO_UPLOAD_SIZE(self):
        return self.MAX_ICON_UPLOAD_SIZE
    ...
    NOBOT_RECAPTCHA_PRIVATE_KEY = env(
        'NOBOT_RECAPTCHA_PRIVATE_KEY', default='')
    ...
    @property
    def ES_URLS(self):
        return [f'http://{host}' for host in self.ES_HOSTS]
    ES_INDEXES = {
        'default': 'addons',
        'stats': 'addons_stats',
    }
    ...
    # Maximum result position. ES defaults to 10000 but we'd like more to make
    # sure all our extensions can be found if searching without a query and
    ...
    # and there's a test verifying it's value is 25000 in
    # amo/test_pagination.py
    ...
    # If this is False, tasks and other jobs that send non-critical emails
    # should use a fake email backend.
    ...
    AUTOGRAPH_CONFIG = {
        ...
        # This is configurable but we don't expect it to be set to anything
        # else but `webextensions-rsa` at this moment because AMO only accepts
    ...
    def STORAGE_ROOT(self):
        return env('NETAPP_STORAGE_ROOT', default=self._path('storage'))

    @property
    def ADDONS_PATH(self):
        return os.path.join(self.STORAGE_ROOT, 'files')
    @property
    def GUARDED_ADDONS_PATH(self):
        return os.path.join(self.STORAGE_ROOT, 'guarded-addons')
    @property
    def GIT_FILE_STORAGE_PATH(self):
        return os.path.join(self.STORAGE_ROOT, 'git-storage')

    @property
    def SHARED_STORAGE(self):
        return os.path.join(self.STORAGE_ROOT, 'shared_storage')

    @property
    def MEDIA_ROOT(self):
        return os.path.join(self.SHARED_STORAGE, 'uploads')
    @property
    def TMP_PATH(self):
        return os.path.join(self.SHARED_STORAGE, 'tmp')
    ...
    # Change this to deactivate API throttling for views using a throttling
    # class depending on the one defined in olympia.api.throttling.
    API_THROTTLING = True
    ...
    def REST_FRAMEWORK(self):
        return {
            ...
            'ALLOWED_VERSIONS': self.DRF_API_VERSIONS,
            'DEFAULT_VERSION': 'v4',
            ...
            'EXCEPTION_HANDLER': (
                'olympia.api.exceptions.custom_exception_handler'),

            # Enable pagination
            'PAGE_SIZE': 25,
            # Use our pagination class by default, which allows clients to
            ...
            # We can't use the default because we don't use django timezone
            # support.
    ...
    def _get_raven_release(self):
        version_json = os.path.join(self.ROOT, 'version.json')
        version = None
        ...
        if not version or version == 'origin/master':
            try:
                version = raven.fetch_git_sha(self.ROOT)
    ...
    def RAVEN_CONFIG(self):
        return {
            ...
            # the current git-sha.
            'release': self._get_raven_release(),
        }
    ...
    # This is the queue used for addons-dev, so it'll consume events (i.e.
    # process then delete) before you can locally.  If you really need to test
    # get ops to stop the 'monitor_fxa_sqs` command.
    ...
    MIGRATED_LWT_DEFAULT_OWNER_EMAIL =(
        'addons-team+landfill-account@mozilla.com')
