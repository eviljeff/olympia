...
def handle_upload_validation_result(results, upload_pk, channel, is_mozilla_signed):
    ...
    statsd.timing(
        f'devhub.validation_results_processed_{quantifier}_1mb', delta
    )
...
def run_addons_linter(path, channel):
    ...
    if not os.path.exists(path):
        raise ValueError(
            f'Path "{path}" is not a file or directory or does not exist.'
        )
    ...
    for addon in addons:
        ...
        for preview in previews:
            try:
                ...
                log.error(
                    f'Failed to find size of preview: {addon.pk}, error: {err}'
                )
