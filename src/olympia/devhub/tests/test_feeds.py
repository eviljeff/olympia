...
class TestActivity(HubTest):
    ...
    def test_xss_addon(self):
        ...
        doc = self.get_pq()
        assert len(doc('.item')) == 1
        assert '<script>' not in text_type(doc), 'XSS FTL'
        assert '&lt;script&gt;' in text_type(doc), 'XSS FTL'
    ...
    def test_xss_tags(self):
        ...
        doc = self.get_pq()
        assert len(doc('.item')) == 1
        assert '<script' not in text_type(doc('.item')), 'XSS FTL'
        assert '&lt;script' in text_type(doc('.item')), 'XSS FTL'
