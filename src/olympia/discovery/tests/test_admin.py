...
class TestDiscoveryAdmin(TestCase):
    ...
    def test_can_change_addon_with_discovery_edit_permission(self):
        ...
        response = self.client.post(
            self.detail_url, {'addon': text_type(addon2.slug)}, follow=True)
        assert response.status_code == 200
        item.reload()
        assert DiscoveryItem.objects.count() == 1
        ...
        response = self.client.post(
            self.detail_url, {'addon': text_type(addon.pk)}, follow=True)
        assert response.status_code == 200
        item.reload()
        assert DiscoveryItem.objects.count() == 1
        assert item.addon == addon
    ...
    def test_change_addon_errors(self):
        ...
        response = self.client.post(
            self.detail_url,
            {'addon': text_type(addon2.pk + 666)}, follow=True)
        assert response.status_code == 200
        assert not response.context_data['adminform'].form.is_valid()
        assert 'addon' in response.context_data['adminform'].form.errors
        item.reload()
        ...
        response = self.client.post(
            self.detail_url, {'addon': text_type(addon3.pk)}, follow=True)
        assert response.status_code == 200
        assert not response.context_data['adminform'].form.is_valid()
        assert 'addon' in response.context_data['adminform'].form.errors
        item.reload()
        ...
        response = self.client.post(
            self.detail_url, {'addon': text_type(addon2.pk)}, follow=True)
        assert response.status_code == 200
        assert not response.context_data['adminform'].form.is_valid()
        assert 'addon' in response.context_data['adminform'].form.errors
        item.reload()
