...
class APITest(TestCase):
    ...
    def test_addon_detail_json(self):
        ...
        assert data['name'] == str(addon.name)
        ...
        assert data['summary'] == str(addon.summary)
        ...
        assert data['eula'] == str(addon.eula)
    ...
    def test_addon_license(self):
        ...
        assert doc('license name').text() == str(license.name)
        assert (
    ...
    def test_15_addon_detail(self):
        ...
        for tag, needle in url_needles.items():
            url = doc(tag).text()
...
class LanguagePacksTest(UploadTest):
    ...
    def test_localepicker(self, get_localepicker):
        get_localepicker.return_value = str('title=اختر لغة', 'utf8')
        self.addon.update(type=amo.ADDON_LPAPP, status=amo.STATUS_PUBLIC)
        res = self.client.get(self.url)
