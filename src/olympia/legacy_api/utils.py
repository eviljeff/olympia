...
def addon_to_dict(addon, disco=False, src='api'):
    ...
    d = {
        'id': addon.id,
        'name': text_type(addon.name) if addon.name else None,
        'guid': addon.guid,
        'status': amo.STATUS_CHOICES_API[addon.status],
        'type': amo.ADDON_SLUGS_UPDATE[addon.type],
        'authors': [{'id': a.id, 'name': text_type(a.name),
                     'link': absolutify(a.get_url_path(src=src))}
                    for a in addon.listed_authors],
        'summary': (
            strip_tags(text_type(addon.summary)) if addon.summary else None),
        'description': strip_tags(text_type(addon.description)),
        'icon': addon.icon_url,
        'learnmore': learnmore,
        ...
        'homepage': text_type(addon.homepage) if addon.homepage else None,
        'support': text_type(addon.support_url) if addon.support_url else None,
    }
    if addon.is_persona():
    ...
    if v:
        d['version'] = v.version
        d['platforms'] = [text_type(a.name) for a in v.supported_platforms]
        d['compatible_apps'] = [{
            text_type(amo.APP_IDS[appver.application].pretty): {
                'min': text_type(appver.min) if appver else (
                    amo.D2C_MIN_VERSIONS.get(app.id, '1.0')),
                'max': (text_type(appver.max) if appver
                        else amo.FAKE_MAX_VERSION),
            }} for app, appver in v.compatible_apps.items() if appver]
    if addon.eula:
        d['eula'] = text_type(addon.eula)

    if addon.developer_comments:
        d['dev_comments'] = text_type(addon.developer_comments)
    ...
    if addon.type == amo.ADDON_PERSONA:
        ...
        def preview_as_dict(preview, src):
            d = {'full': urlparams(preview.image_url, src=src),
                 ...
                 'caption': text_type(preview.caption)}
            return d
