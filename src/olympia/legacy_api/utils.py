...
def addon_to_dict(addon, disco=False, src='api'):
    ...
    d = {
        'id': addon.id,
        'name': str(addon.name) if addon.name else None,
        'guid': addon.guid,
        'status': amo.STATUS_CHOICES_API[addon.status],
        'type': amo.ADDON_SLUGS_UPDATE[addon.type],
        'authors': [{'id': a.id, 'name': str(a.name),
                     'link': absolutify(a.get_url_path(src=src))}
                    for a in addon.listed_authors],
        'summary': (
            strip_tags(str(addon.summary)) if addon.summary else None),
        'description': strip_tags(str(addon.description)),
        'icon': addon.icon_url,
        'learnmore': learnmore,
        ...
        'homepage': str(addon.homepage) if addon.homepage else None,
        'support': str(addon.support_url) if addon.support_url else None,
    }
    if addon.is_persona():
    ...
    if v:
        d['version'] = v.version
        d['platforms'] = [str(a.name) for a in v.supported_platforms]
        d['compatible_apps'] = [{
            str(amo.APP_IDS[appver.application].pretty): {
                'min': str(appver.min) if appver else (
                    amo.D2C_MIN_VERSIONS.get(app.id, '1.0')),
                'max': str(appver.max) if appver else amo.FAKE_MAX_VERSION,
            }} for app, appver in list(v.compatible_apps.items()) if appver]
    if addon.eula:
        d['eula'] = str(addon.eula)

    if addon.developer_comments:
        d['dev_comments'] = str(addon.developer_comments)
