...
RAW_FILTER_PATTERN = re.compile(
    r'^(?P<field>[a-zA-Z0-9_\.]+)\s*(?P<op>=|>|<|>=|<=|!=|IN|LIKE|ILIKE)\s*$', re.I
)
...
class RawSQLManager:
    ...
    def get(self, **kw):
        ...
        if cnt > 1:
            ...
            raise clone.sql_model.DoesNotExist(
                '%s matching query does not exist.' % self.sql_model.__class__.__name__
            )
        else:
            return clone[0:1][0]
    ...
    def filter(self, *args, **kw):
        ...
        for arg in args:
            if isinstance(arg, Q):
                ...
                raise TypeError('non keyword args should be Q objects, got %r' % arg)
        for field, val in kw.items():
            clone.base_query['where'].append(clone._kw_filter_to_clause(field, val))
        return clone
    ...
    def order_by(self, spec):
        ...
        clone.base_query['order_by'].append(
            f'{clone._resolve_alias(field)} {dir}'
        )
        return clone
    ...
    def _clone(self):
        return self.__class__(self.sql_model, base_query=copy.deepcopy(self.base_query))
    ...
    def _filter_to_clause(self, *specs, **kw):
        ...
        specs = list(specs)
        if (len(specs) % 2) != 0:
            raise TypeError("Expected pairs of 'spec =', 'val'. Got: %r" % specs)
        full_clause = []
        while len(specs):
    ...
    def _compile(self, parts):
        ...
        if len(parts['limit']):
            stmt = '{}\nLIMIT {}'.format(stmt, ', '.join([str(i) for i in parts['limit']]))
        return stmt
