...
def review(request, addon, channel=None):
    ...
    if content_review_only:
        # Content review ignores beta and disabled: they don't need to be
        # reviewed, betas are always public and disabled will revert to their
        # original status if they are ever re-enabled.
        version = addon.find_latest_version(channel=channel)
    else:
        # Non-content review needs to include disabled as rejecting a version
        # makes the files disabled and we might need to follow-up on them.
