...
def themes_list(request, flagged=False, rereview=False):
    ...
    return render(request, 'reviewers/themes/queue_list.html', context(
        **{'addons': pager.object_list,
           ...
           'statuses': dict((k, str(v)) for k, v in
                            list(amo.STATUS_CHOICES_API.items())),
...
def _themes_queue(request, flagged=False, rereview=False):
    ...
    return render(request, 'reviewers/themes/queue.html', context(
...
def _get_themes(request, reviewer, flagged=False, rereview=False):
    ...
    if not themes:
        expired_locks = ThemeLock.objects.filter(
            ...
            theme__addon__status=status)[:amo.THEME_INITIAL_LOCKS]
        # Steal expired locks.
