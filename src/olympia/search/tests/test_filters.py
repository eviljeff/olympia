...
class TestSearchParameterFilter(FilterTestsBase):
    ...
    def test_search_by_type_invalid(self):
        with self.assertRaises(serializers.ValidationError) as context:
            self._filter(data={'type': text_type(amo.ADDON_EXTENSION + 666)})
    ...
    def test_search_by_type_id(self):
        qs = self._filter(data={'type': text_type(amo.ADDON_EXTENSION)})
        must = qs['query']['bool']['must']
        assert {'terms': {'type': [amo.ADDON_EXTENSION]}} in must

        qs = self._filter(data={'type': text_type(amo.ADDON_PERSONA)})
        must = qs['query']['bool']['must']
    ...
    def test_search_by_app_invalid(self):
        with self.assertRaises(serializers.ValidationError) as context:
            self._filter(data={'app': text_type(amo.FIREFOX.id + 666)})
    ...
    def test_search_by_app_id(self):
        qs = self._filter(data={'app': text_type(amo.FIREFOX.id)})
        must = qs['query']['bool']['must']
        assert {'term': {'app': amo.FIREFOX.id}} in must

        qs = self._filter(data={'app': text_type(amo.THUNDERBIRD.id)})
        must = qs['query']['bool']['must']
    ...
    def test_search_by_platform_invalid(self):
        with self.assertRaises(serializers.ValidationError) as context:
            self._filter(
                data={'platform': text_type(amo.PLATFORM_WIN.id + 42)})
    ...
    def test_search_by_platform_id(self):
        qs = self._filter(data={'platform': text_type(amo.PLATFORM_WIN.id)})
        must = qs['query']['bool']['must']
        assert {'terms': {'platforms': [
            amo.PLATFORM_WIN.id, amo.PLATFORM_ALL.id]}} in must

        qs = self._filter(data={'platform': text_type(amo.PLATFORM_LINUX.id)})
        must = qs['query']['bool']['must']
        assert {'terms': {'platforms': [
