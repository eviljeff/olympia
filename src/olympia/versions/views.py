...
def download_file(request, file_id, type=None, file_=None, addon=None):
    ...
    if (addon.is_disabled or file_.status == amo.STATUS_DISABLED or
            channel == amo.RELEASE_CHANNEL_UNLISTED):

        is_editor = (acl.check_addons_reviewer(request)
                     if channel == amo.RELEASE_CHANNEL_LISTED
                     else acl.check_unlisted_addons_reviewer(request))
        if is_editor or acl.check_addon_ownership(
            ...
            log.info(
                u'download file {file_id}: addon/file disabled or listed and '
                u'user {user_id} is not an owner or editor.'.format(
                    file_id=file_id, user_id=request.user.pk))
            raise http.Http404()  # Listed, not owner or admin.
